/**
 * Server-side API endpoint for AI Question Generation
...
const SITE_URL = import.meta.env.FRONTEND_URL || import.meta.env.SITE || 'http://localhost:4321';

// Question pools for fallback
...
function getRandomFallbackQuestion(sessionType: string, previousQuestions: string[] = []): string {
    ...
    if (availableQuestions.length === 0) {
        // If all questions used, return a random one from the full pool
...
function buildSingleQuestionPrompt(sessionType: string, previousQuestions: string[] = []): string {
...
- Make it unique and different from typical questions`;
    ...
    if (previousQuestions.length > 0) {
        const recentQuestions = previousQuestions.slice(-5); // Only include last 5 to keep context small
